...
especially for complicated operations. **`dplyr`** is a
...
Each line in the file holds the number of reads overlapping a
...
1. the data
2. *id_cols* with the set of variables that uniquely identifies each
   observation
3. *names_from* variable to get the output column names from
4. *values_from* column variable whose values will fill the new column
   variables
...
1. the data
2. *cols* with the set of columns we want transform into a variable
3. *names_to* column variable we wish to create and fill with values 
associated with the key
4. *values_to* column variable we want to fill with the values
